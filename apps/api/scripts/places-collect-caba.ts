/**
 * Colecta place_ids en los 48 barrios oficiales de CABA × keywords.
 * Sin Details. Dedupe global en places-registry.json. Solo OPERATIONAL.
 *
 * Keywords:
 *   - sala de ensayo
 ...
 *
 * Query: "<keyword> <barrio> CABA"
 *
 * Uso:
 *   GOOGLE_MAPS_API_KEY=... pnpm exec tsx scripts/places-collect-caba.ts
 *   pnpm exec tsx scripts/places-collect-caba.ts --keyword="estudio de grabación"
 *
 * Opcional:
 *   PLACES_DELAY_MS=400  (default 350 entre requests)
...
config({ path: path.resolve(__dirnameEnv, "../../../.env") });
config({ path: path.resolve(__dirnameEnv, "../../app/.env.local") });
...
async function main() {
  if (BARRIOS.length !== 48) {
    throw new Error(`Se esperaban 48 barrios, hay ${BARRIOS.length}`);
  }

  const totalRuns = BARRIOS.length * KEYWORDS.length;
  const registry = await loadRegistry();
  const knownBefore = Object.keys(registry.places).length;

  console.log(`→ CABA batch collect (sin Details)`);
  console.log(`  barrios: ${BARRIOS.length}`);
  console.log(`  keywords: ${KEYWORDS.length} → ${KEYWORDS.join(" · ")}`);
  console.log(`  corridas: ${totalRuns} (${BARRIOS.length}×${KEYWORDS.length})`);
  console.log(`  query: "<kw> <barrio> CABA"`);
  ...
  let knownTotal = 0;
  let errors = 0;
  const perBarrio: Record<
    string,
    { api: number; operational: number; nuevos: number; known: number }
  > = {};

  for (const barrio of BARRIOS) {
    perBarrio[barrio] = { api: 0, operational: 0, nuevos: 0, known: 0 };
    ...
    for (const keyword of KEYWORDS) {
      ...
      process.stdout.write(
        `[${run}/${totalRuns}] ${keyword} · ${barrio} CABA … `,
      );

      try {
        const rows = await searchText(textQuery);
        const operational = rows.filter((p) => p.businessStatus === "OPERATIONAL");
        apiHits += rows.length;
        operationalHits += operational.length;
        perBarrio[barrio]!.api += rows.length;
        perBarrio[barrio]!.operational += operational.length;
        ...
        perBarrio[barrio]!.nuevos += nuevos;
        perBarrio[barrio]!.known += known;
      ...
      // guardado parcial cada 8 corridas
      if (run % 8 === 0) {
  ...
  const summaryPath = path.join(
    OUT_DIR,
    `collect-caba-${summary.ranAt.slice(0, 10)}.json`,
  );
  ...
  console.log(
    `Registry: ${knownBefore} → ${Object.keys(registry.places).length} ids`,
  );
